 ...
 */
int Mp3Code::inner_loop(int ix[samp_per_frame2],int max_bits,gr_info *gi)
{
 ...
 * ------------------
 */
void Mp3Code::L3_iteration_loop(long            mdct_freq_org[2][2][samp_per_frame2],
                                ...
                                int             mean_bits)
{
  ...
  for(gr=0; gr<m_Mpeg.granules; gr++)
    for(ch=m_Mpeg.channels; ch--; )
    {
      ...
      for ( i = 0; i < m_iCutOff; i++ )
        if (xr[i] < 0)
          ix[i] *= -1;
    }

  /* Sort out reservoir at frame end, limit the size and stuff the excess */
  resv_max = DEC_BUFF_LIMIT - m_Mpeg.bits_per_frame;
  if(resv_max > m_Mpeg.resv_limit)
    resv_max = m_Mpeg.resv_limit;
  else if(resv_max < 0)
    resv_max = 0;
 ...
 */
int Mp3Code::quantize(int ix[samp_per_frame2], int stepsize )
{
 ...
 */
int Mp3Code::ix_max( int ix[samp_per_frame2], unsigned int begin, unsigned int end )
{
 ...
 */
void Mp3Code::calc_runlen( int ix[samp_per_frame2], gr_info *gi )
{
  int i;

  /* zeros */
  for ( i = m_iCutOff; i > 1; i -= 2 )
    if (ix[i-1] | ix[i-2])
      break;
 ...
 */
int Mp3Code::count1_bitcount(int ix[samp_per_frame2], gr_info *gi)
{
  unsigned int p, i, k;
 ...
 */
void Mp3Code::subdivide(gr_info *gi)
{
  static struct
  {
 ...
 */
void Mp3Code::bigv_tab_select( int ix[samp_per_frame2], gr_info *gi )
{
  gi->table_select[0] = 0;
  gi->table_select[1] = 0;
  gi->table_select[2] = 0;
 ...
 */
int Mp3Code::bigv_bitcount(int ix[samp_per_frame2], gr_info *gi)
{
  int bits = 0;
  unsigned int table;
 ...
 */
int Mp3Code::new_choose_table( int ix[samp_per_frame2], unsigned int begin, unsigned int end )
{
  unsigned int i, max;
  int choice[2];
  int sum[2];
 ...
 */
int Mp3Code::count_bit(int ix[samp_per_frame2],
              unsigned int start,
              unsigned int end,
              unsigned int table )
{
