 ...
 */
int Mp3Code::putbytes(unsigned char *data, int n)
{
	unsigned int free = OutData.iBufSize - OutData.iBufCount;
	int iRetBufIndex = -1;

    if((unsigned int)n >= free)
    {
		memcpy(&(OutData.pOutBuf[OutData.iBufIndex][OutData.iBufCount]),data,free);
        iRetBufIndex = OutData.iBufIndex;
		n -= free;
		OutData.iBufIndex = OutData.iBufIndex ? 0:1;
		OutData.iBufCount = 0;
        memcpy(&(OutData.pOutBuf[OutData.iBufIndex][OutData.iBufCount]),&data[free],n);
        OutData.iBufCount = n;
    }
    else
    {
        memcpy(&(OutData.pOutBuf[OutData.iBufIndex][OutData.iBufCount]),data,n);
        OutData.iBufCount += n;
    }
	return iRetBufIndex;
}
...
void Mp3Code::L3_format_bitstream( int l3_enc[2][2][samp_per_frame2], L3_side_info_t  *l3_side)
{
  int main_bytes;
 ...
 */
void Mp3Code::putbits (unsigned char *buf, unsigned long val, unsigned int n)
{
    ...
    while (n)
    {
        k = ((unsigned int)n < (unsigned int)bi) ? n : bi;
        tmp = val >> (n - k);
 ...
 */
int Mp3Code::encodeMainData(int l3_enc[2][2][samp_per_frame2],L3_side_info_t  *si)
{
  int gr, ch;
 ...
 * in the bitstream.
 */
void Mp3Code::encodeSideInfo( L3_side_info_t  *si )
{
  int gr, ch, region;
  ...
  *sf     = header[0];
  *(sf+1) = header[1];
  *(sf+2) = header[2] | (m_Mpeg.padding << 1);
  *(sf+3) = header[3];
  *(sf+4) = 0;
  ...
  if(m_Mpeg.type == MPEG1)
  {
    ...
    putbits(sf, 0, ( m_Mpeg.channels == 2 ) ? 3 : 5 ); /* private bits */
  ...
  else /* mpeg 2/2.5 */
  {
    ...
    putbits(sf, 0, ( m_Mpeg.channels == 1 ) ? 1 : 2 ); /* private bits */
  ...
  fifo[wr].fr_len = (m_Mpeg.bits_per_frame - bits) >> 3; /* data bytes in this frame */
 ...
 */
void Mp3Code::Huffmancodebits( int *ix, gr_info *gi )
{
  int region1Start;
  int region2Start;
 ...
 * -----------------------
 */
int Mp3Code::L3_huffman_coder_count1( struct huffcodetab *h, int v, int w, int x, int y )
{
  HUFFBITS huffbits;
 ...
 */
int Mp3Code::HuffmanCode(int table_select, int x, int y, unsigned long *code,
...
int Mp3Code::EndData(void)
{
   int iBufNum = 0,iTemp;
   if(count)
       iBufNum = setbytes(0,count); /* fill current frame */

   while(rd != wr)
   {
      /* output next header/sideinfo */
      putbytes(fifo[rd].side, fifo[rd].si_len);
      iTemp = setbytes(0, fifo[rd].fr_len); /* fill frame */
	  if(iTemp>iBufNum)
		  iBufNum = iTemp;
      if(++rd == FIFO_SIZE) rd = 0; /* point to next header/sideinfo */
   }
 ...
 */
int Mp3Code::setbytes(unsigned char data, int n)
{
	unsigned int free = OutData.iBufSize - OutData.iBufCount;
	int iBufNum = 1;
    ...
    if((unsigned int)n >= free)
    {
		memset(&(OutData.pOutBuf[OutData.iBufIndex][OutData.iBufCount]),data,free);
        iBufNum = 2;
        n -= free;
		OutData.iBufIndex = OutData.iBufIndex ? 0:1;
        memset(&(OutData.pOutBuf[OutData.iBufIndex][OutData.iBufCount]),data,n);
		OutData.iBufCount = n;
    }
    else
    {
