...
// 读取硬盘序列号函数
char* CGetHDSerial1::GetHDSerial()
{
    m_buffer[0] = '\0';
    // 得到当前操作系统版本
    ...
    if (OSVersionInfo.dwPlatformId != VER_PLATFORM_WIN32_NT)
    {
        ...
        memset(m_wWin9xHDSerial, 0, sizeof(m_wWin9xHDSerial));
    ...
    else
    {
        ...
        memset(m_wWinNTHDSerial, 0, sizeof(m_wWinNTHDSerial));
        // 判断是否有SCSI硬盘
...
void _stdcall CGetHDSerial1::Win9xReadHDSerial(WORD * buffer)
{
    int i;
    for (i = 0; i < 256; i++)
        buffer[i] = 0;
...
char * CGetHDSerial1::WORDToChar(WORD diskdata[256], int firstIndex, int lastIndex)
{
...
char* CGetHDSerial1::DWORDToChar(DWORD diskdata[256], int firstIndex, int lastIndex)
{
    static char string[1024];
    int index = 0;
    int position = 0;
...
BOOL CGetHDSerial1::WinNTReadIDEHDSerial(DWORD * buffer)
{
    m_buffer[0] = 0;
...
BOOL CGetHDSerial1::WinNTReadSCSIHDSerial(DWORD * buffer)
{
    buffer[0] = '\0';
        int controller = 0;
...
BOOL CGetHDSerial1::WinNTGetIDEHDInfo(HANDLE hPhysicalDriveIOCTL, PSENDCMDINPARAMS pSCIP,
